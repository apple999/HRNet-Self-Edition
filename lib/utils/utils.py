...
def create_logger(cfg, cfg_name, phase='train'):
    ...
    cfg_name = cfg_name + "_" + str(cfg.MODEL.NUM_JOINTS)   # 区分wflw98还是wflw65
    ...
    #                     (cfg_name + '_' + time_str)     # 保存的log文件名
    tensorboard_log_dir = Path(cfg.LOG_DIR) / dataset / model / cfg_name  # 保存的log文件名
...
def save_checkpoint(states, predictions, is_best,
    ...
    if os.access(os.path.join(output_dir, filename), os.R_OK):  # 这里一定要用绝对路径，要不然会打不开
